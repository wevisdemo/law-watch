...
csv()
	...
	.then((arr) => {
		...
		const markedRefArr = formattedArr.map((e) => {
			...
			if (e.Law_Merge_Head) {
				e.Law_Keyword = [e.Law_Keyword, ...ref_keyword[e.Law_ID].map((e) => e[1])]
					.join(' ')
					.toLocaleLowerCase();
			}
			if (e.Law_Merge) {
		...
		for (let cc of [by_party, by_cabinet, by_people]) {
			if ('2018' in cc) {
				for (let k in cc['2018']) {
					if (k in cc['2019']) {
						cc['2019'][k][0] += cc['2018'][k][0];
						cc['2019'][k][1] += cc['2018'][k][1];
					} else {
						cc['2019'][k] = cc['2018'][k];
					}
				}
